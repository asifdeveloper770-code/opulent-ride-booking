...
export function BookingForm({ compact = false }: { compact?: boolean }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Reservation request received — our concierge will confirm shortly.");
      setData(initial);
    } catch (error) {
  ...
  return (
    <form onSubmit={submit} className={`relative bg-card/80 backdrop-blur border border-border ${compact ? "p-6" : "p-8 md:p-10"} shadow-elegant`}>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <BookingField {...fieldProps} name="serviceType" value={data.serviceType} error={errors.serviceType} options={["Airport Transfer","Point-to-Point","Hourly Charter","Wedding","Corporate","Special Event","Prom","Night Out", "Others"]}>
          <span className="inline-flex items-center gap-2"><Car className="size-3.5" /> Service Type</span>
        </BookingField>
        <BookingField {...fieldProps} name="vehicleType" value={data.vehicleType} error={errors.vehicleType} options={["Executive SUV","Stretch Limousine","Sprinter Van","Party Bus"]}>
          <span className="inline-flex items-center gap-2"><Car className="size-3.5" /> Vehicle</span>
        </BookingField>

        <BookingField {...fieldProps} name="pickupLocation" value={data.pickupLocation} error={errors.pickupLocation} placeholder="Address, hotel or airport">
          <span className="inline-flex items-center gap-2"><MapPin className="size-3.5" /> Pickup Location</span>
        </BookingField>
        <BookingField {...fieldProps} name="dropoffLocation" value={data.dropoffLocation} error={errors.dropoffLocation} placeholder="Destination address">
          <span className="inline-flex items-center gap-2"><MapPin className="size-3.5" /> Drop-off Location</span>
        </BookingField>

        <BookingField {...fieldProps} name="pickupDate" value={data.pickupDate} error={errors.pickupDate} type="date">
          <span className="inline-flex items-center gap-2"><Calendar className="size-3.5" /> Pickup Date</span>
        </BookingField>
        <BookingField {...fieldProps} name="pickupTime" value={data.pickupTime} error={errors.pickupTime} type="time">
          <span className="inline-flex items-center gap-2"><Clock className="size-3.5" /> Pickup Time</span>
        </BookingField>

        <BookingField {...fieldProps} name="passengers" value={data.passengers} error={errors.passengers} type="number">
          <span className="inline-flex items-center gap-2"><Users className="size-3.5" /> Passengers</span>
        </BookingField>
        <BookingField {...fieldProps} name="luggage" value={data.luggage} error={errors.luggage} type="number">
          <span className="inline-flex items-center gap-2"><Users className="size-3.5" /> Luggage Pieces</span>
        </BookingField>

        <BookingField {...fieldProps} name="firstName" value={data.firstName} error={errors.firstName} placeholder="John"><span className="inline-flex items-center gap-2"><User className="size-3.5" /> First Name</span></BookingField>
        <BookingField {...fieldProps} name="lastName" value={data.lastName} error={errors.lastName} placeholder="Doe"><span className="inline-flex items-center gap-2"><User className="size-3.5" /> Last Name</span></BookingField>

        <BookingField {...fieldProps} name="email" value={data.email} error={errors.email} type="email" placeholder="john@example.com"><span className="inline-flex items-center gap-2"><Mail className="size-3.5" /> Email</span></BookingField>
        <BookingField {...fieldProps} name="phone" value={data.phone} error={errors.phone} type="tel" placeholder="+1 (555) 123-4567"><span className="inline-flex items-center gap-2"><Phone className="size-3.5" /> Phone</span></BookingField>

        <BookingField {...fieldProps} name="flightNumber" value={data.flightNumber} error={errors.flightNumber} placeholder="Optional"><span className="inline-flex items-center gap-2">Flight Number</span></BookingField>
        <div />

        <div className="md:col-span-2">
          <label className={label}><span className="inline-flex items-center gap-2"><MessageSquare className="size-3.5" /> Special Requests</span></label>
          <textarea
      ...
      >
        <span className="relative z-10">{isSubmitting ? "Sending..." : "Reserve My Chauffeur"}</span>
